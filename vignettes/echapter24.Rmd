...
evaluate.ADDT.test.plan(ADDT.plan.values = InsulationBrkdwn.ADDTpv,
...
plot.ADDT.sample.size(ADDT.plan.values = InsulationBrkdwn.ADDTpv,
                      ADDT.test.plan = InsulationBrkdwn.ADDTplan,
                      use.condition = "150",
                      FailLevel = 10,
                      quantile.of.interest = 0.2)
...
InsulationBrkdwn.test.ADDTplan <- get.allocation.matrix(list(DegreesC = c(180,225,250,275)),
                                                        times = c(1,2,4,8,16,32,48,64),
                                                        time.units = "Weeks",
                                                        reps = 400)

print.ADDT.test.plan(InsulationBrkdwn.test.ADDTplan)
plot.ADDT.test.plan(InsulationBrkdwn.test.ADDTplan)
...
```{r}
InsulationBrkdwn.test.ADDTpv <-get.ADDT.plan.values(distribution = "normal",
                                                    transformation.x = "Arrhenius",
                                                    transformation.Response = "log",
                                                    transformation.time = "linear",
                                                    beta0 = 2.58850162033243,
...
evaluate.ADDT.test.plan(ADDT.plan.values = InsulationBrkdwn.test.ADDTpv,
                        ADDT.test.plan = InsulationBrkdwn.test.ADDTplan,
                        use.condition = "150",
                        FailLevel = 10,
                        quantile.of.interest = c(0.1,.2,.3,.4,.9))
...
BondTest.ADDTplan <-get.allocation.matrix(list(DegreesC = c(50,80,100,120)),
                                          times = c(1,2,4,8,16,32,48,64),
                                          time.units = "Weeks",
                                          reps = 10)

Filament.ADDTplan  <- get.allocation.matrix(list(volts = c(5,10),
                                                 DegreesC = c(40,  50,60)),
                                            times = c(1,2,5,10,20,50),
                                            time.units = "Weeks")

#define from a frame
Filament.hframe <-read.table(SplidaDataName("Filament_hframe.txt"), head = T)
Filament.hframe.ADDTplan  <- get.ADDT.test.plan.hframe(Filament.hframe,
                                                       levels.columns = c(1,2), 
                                                       time.columns = 3:7)

StrengthCompare.ADDTplan  <- get.allocation.matrix(list(DegreesC = c(50,60,70),
                                                        Type = c("Type1","Type2")),
                                                   times = c(1,2,5,10,20,50),
                                                   time.units = "Weeks")
...
AdhesiveBondB.SmallestExtremeValue.ADDTpv <-get.ADDT.plan.values(distribution = "normal",
...
AdhesiveBondB.ADDTplan <- get.allocation.matrix(list(DegreesC = c(25,50,60,70)),
                                                times = c(0,2,4,6,12,16),
                                                time.units = "Weeks",
                                                reps = 6)

ADDT.vcv(ADDT.plan.values = AdhesiveBondB.SmallestExtremeValue.ADDTpv,
         ADDT.test.plan = hframe.to.vframe(AdhesiveBondB.ADDTplan))

last.out <-evaluate.ADDT.test.plan(ADDT.plan.values = AdhesiveBondB.SmallestExtremeValue.ADDTpv, 
...
AdhesiveBondC.ADDTpv <-get.ADDT.plan.values(distribution = "sev",
...
AdhesiveBondC.ADDTplan  <- get.allocation.matrix(list(DegreesC = c(40, 50,60),
                                                      RH = c(20,80)),
                                                 times = c(1,2,5,10,20,50),
                                                 time.units = "Weeks",
                                                 reps = 6)

tmp.pmodel <-  pseudo.model(ADDT.plan.values = AdhesiveBondC.ADDTpv,
                            ADDT.test.plan = hframe.to.vframe(AdhesiveBondC.ADDTplan))

f.ADDT.stableparam(AdhesiveBondC.ADDTpv$theta.vec, tmp.pmodel)
...
evaluate.ADDT.test.plan(ADDT.plan.values = AdhesiveBondC.ADDTpv,
                        ADDT.test.plan = AdhesiveBondC.ADDTplan,
                        quantile.of.interest = c(0.1,.2,.3,.4,.9))

plot.ADDT.sample.size(ADDT.plan.values = AdhesiveBondC.ADDTpv,
                      ADDT.test.plan = AdhesiveBondC.ADDTplan,
                      quantile.of.interest = 0.2)

AdhesiveBondC.vframe <- hframe.to.vframe(AdhesiveBondC.ADDTplan)

AdhesiveBondC.sim.out <- simulate.ADDT.test.plan(ADDT.test.plan = AdhesiveBondC.ADDTplan, 
...
TestAdhesiveBondB.SmallestExtremeValue.ADDTpv <-get.ADDT.plan.values(distribution = "sev",
                                                                     transformation.x = "Arrhenius", 
                                                                     transformation.Response = "log",
                                                                     transformation.time = "Square root", 
                                                                     beta0 = 2.168,
...
evaluate.ADDT.test.plan(ADDT.plan.values = TestAdhesiveBondB.SmallestExtremeValue.ADDTpv,
                        ADDT.test.plan = AdhesiveBondB.ADDTplan,
                        quantile.of.interest = c(0.1,.2,.3,.4,.9))

AdhesiveBondB.plan.sim.out <- simulate.ADDT.test.plan(ADDT.test.plan = AdhesiveBondB.ADDTplan, 
                                                      ADDT.plan.values = AdhesiveBondB.SmallestExtremeValue.ADDTpv, 
                                                      number.sim = 6, 
                                                      plotem = 5)
