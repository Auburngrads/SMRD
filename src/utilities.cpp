...
Rcpp::IntegerVector merge_sortd(Rcpp::NumericVector x,
  ...
  if(iorder.at(n - 1) - iorder.at(0) < 0.0001) {
    ...
    for(int j = 1; j <= n; j++){
        ...
        tempmat.row(j - 1) = clone(x);

    }

    for(int i = 1; i <= n; i++) {

      rowi = tempmat.row(i - 1);
      tmin = which_min(rowi);
      tempvec.at(tmin) = i;
    ...
    for(int j = 1; j <= n; j++){
        ...
        tempmat2.row(j - 1) = clone(tempvec);
  ...
  for(int i = 1; i <= n; i++) {
      ...
      tempvec2 = tempmat2.row(i - 1);
      tmin = which_min(tempvec2);
      key.at(i - 1) = iorder.at(tmin);
...
void merge_sortdd(Rcpp::NumericVector &xkey, 
                  int nvalp, 
                  Rcpp::IntegerVector &iorder, 
...
// one steps in the c subset, that we create in the final rank array;
   ...
   // iorder (iwrkd:iwrkf) = iscrat (iwrkd:iwrkf);
      for(int i = iwrkd; i <= iwrkf; i++){
        
          iorder.at(i - 1) = iscrat.at(i - 1);
...
void merge_sortii(Rcpp::IntegerVector &xkey, 
                 int nvalp, 
                 Rcpp::IntegerVector &iorder, 
                 Rcpp::IntegerVector &iscrat){
