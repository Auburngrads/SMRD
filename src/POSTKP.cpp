...
//   beta parameters for binomial probs
double pin = float(nsamsz - kord + 1);
double qin = float(kord);
double phib = 0,phibm = 0,phis = 0,phip = 0;
double z, factx, bicofl, pdflog, pdfij;
...
// Loop over the estimates from the posterior sample
for(int j = 0; j < nsim; j++){
  
    z = (xltime.at(i) - xmu.at(j)) / sigma.at(j);
    factx = -1 * std::log(sigma.at(j));
    
    if((kdist % 2) == 0) {
      
       factx = -1 * xltime.at(i) - std::log(sigma.at(j));
      
    }
    
    wqm_phiall(phib,phibm,phis,phip,z,kdist);

bicofl = flnf(nsamsz) - flnf(kord) - flnf(nsamsz - kord);

pdflog = bicofl + float(kord - 1) * std::log(phib) + \
         std::log(phis) + factx + float(nsamsz - kord) * \
         std::log(phibm);

// Got some -Inf from log(0), but when combined, 
// Gave NaN and this choked dexp
// Might need a more elaborate trap here
   pdfij = zero;

if(pdflog < 70.0e00) pdfij = std::exp(pdflog);

pdf.at(i) = pdf.at(i) + pdfij;
cdf.at(i) = cdf.at(i) + R::pbeta(phib, qin, pin, 1, 0);
...
cdf.at(i) = cdf.at(i) / float(nsim);
  ...
  return Rcpp::List::create(Named("pdf") = pdf,
                            Named("cdf") = cdf);
