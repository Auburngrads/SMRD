...
// specify the number failing in each group
//
...
// Loop across the groups
   ...
   // kctype = 2 for type 2 censored data
      kctype = 2;
      smldat(theta,nparm,nsamsz,kctype,centim,
             pretim,kdist,x,y,cen,wt,nrleft,nter,ny,nty,
             ty,tcodes,nrownw,krfail,igroup,
             iobs,kpred,iersim);
   ...
   if((iersim > 0) & (debug::kprint > 0)) {
      ...
      Rcpp::Rcout << "\nmsmdat2 error\n"                  << std::endl;
      Rcpp::Rcout << "igroup"           << igroup         << std::endl;
      Rcpp::Rcout << "nsamsz(igroup)"   << nsamsz(igroup) << std::endl;
