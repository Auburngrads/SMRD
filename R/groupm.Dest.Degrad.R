...
function (data.ddd, transformation.response, transformation.time,
    ...
    add = F, grids = F, title.option = GetSMRDDefault("SMRD.TitleOption"), do.legend = "On plot",
    subset = T, pch.point = NULL, response.on.yaxis = T, new.data = NULL,
    FailLevel = NULL, power = NULL, PlotFailDefLine = T,debug1= map.SMRDDebugLevel(),
    lty = NULL, ...)
{
  ...
  ADDTZeroRepsFilter <-
    ...
    the.do.list.orig <- get.x.markers(data.ddd, group.var = group.var,
    ...
    trans.data.ddd <- plot.Dest.Degrad.data(x = data.ddd,
        ...
        do.legend = "suppress", group.var = group.var)
    gmle.out <- dest.degrad.mle(data.ddd, group.var = group.var,
        distribution = distribution, transformation.response = transformation.response,
        transformation.time = transformation.time, transformation.x = transformation.x,
        power = power,debug1= debug1)
    if (!is.null(new.data)) {
        ...
        the.do.list.new.data <- get.x.markers(new.data, sending.xmat = T,
            long = T, include.complete = T)
    ...
    if (is.null(lty)) {
        ...
        else lty <- (1:(length(the.do.list) + 1))[-2]
    }
    leave.symbol.out <- c(2, 6, 17, 19)
    if (is.null(pch.point))
        pch.point <- (1:(length(the.do.list) + length(leave.symbol.out)))[-leave.symbol.out]
    sub.model <- get.sub.model.dest.degrad.mle.out(gmle.out,
        stresses = stresses.plus)
    gmle.out$residuals <- get.residuals(gmle.out)
    x.columns(data.ddd) <- c(get.x.columns(data.ddd), get.time.column(data.ddd))
    gmle.out$data.ld <- data.ddd
    gmle.out$the.orig.data.ld <- data.ddd
    gmle.out$analysis.type <- "Destructive Degradation"
    x.names <- (dimnames(the.xmat)[[2]])
    gmle.out$relationship <- paste("Resp:", gmle.out$model$transformation.response,
        ",Time:", gmle.out$model$transformation.time, ",", paste(x.names,
            transformation.x, collapse = ",", sep = ":"), sep = "")
    relationship.string <- gmle.out$relationship
    gmle.out$distribution <- gmle.out$model$distribution
    if (map.SMRDDebugLevel() >= 4)
        cat("number of char in relationships=", nchar(relationship.string),
            "\n")
    if (nchar(relationship.string) > 50)
        relationship.string <- paste("Resp:", gmle.out$model$transformation.response,
            ",Time:", gmle.out$model$transformation.time, ",",
            paste(abbreviate(x.names), abbreviate(transformation.x),
                collapse = ",", sep = ":"), sep = "")
    ...
    for (i in 1:length(slope)) {
        cat("Model slope at", the.do.list[i], "Intercept=", format(intercept[i]),
            "slope=", format(slope[i]), "\n")
        abline(intercept[i], slope[i], col = i, lty = lty[i],
            lwd = 2)
    }
    ...
    if (is.null(my.title)) {
        my.title <- paste(get.data.title(trans.data.ddd), "\n",
            gmle.out$analysis.type, " Regression Analyses\n",
            relationship.string, paste(", Dist:", gmle.out$model$distribution,
                sep = ""), sep = "")
    }
    ...
    if (CheckString("full", title.option))
        mtext(text = my.title, side = 3, cex = 1.2, line = title.line.adj)
    if (mean(slope) > 0) {
        legend.x <- x.loc(0.55)
        legend.y <- y.loc(0.99)
  } else {
    ...
    the.pch.point[1:length(the.do.list.orig)] <- -the.pch.point[1:length(the.do.list.orig)]
        ...
        if (do.legend == "On plot")
            legend(legend.x, legend.y, the.do.list, cex = 1.1,
                bty = "n", col = 1:length(the.do.list), pch = the.pch.point[1:length(the.do.list)],
        ...
        if (do.legend == "New page" || do.legend == "New file") {
            ...
            plot(c(0, 0), c(1, 1), xlab = "", ylab = "", type = "n",
                xaxt = "n", yaxt = "n")
            legend(x.loc(0.003), y.loc(0.994), the.do.list, cex = 1.1,
                bty = "n", col = 1:length(the.do.list), pch = the.pch.point[1:length(the.do.list)],
                lty = lty,y.intersp = 0.675)
            if (do.legend == "New file")
                dev.off()
