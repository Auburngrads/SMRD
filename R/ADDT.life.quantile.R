ADDT.life.quantile <-
function (p, distribution, FailLevel.tran, xuse.tran, theta.hat) 
{
    if (nrow(xuse.tran) > 1) {
    ...
    if (increasing) {
        pnow <- 1 - p
        good.ones <- 1 - wqmf.phibf(-mu.quantile/sigma.quantile, 
            distribution) < p
        answers[good.ones] <- -(mu.quantile + quant(pnow[good.ones], 
            distribution) * sigma.quantile)
    }
    else {
        pnow <- p
