          ...
          printem = T,...)
{
    ...
    length.to <- min(length(prob.vec[prob.vec <= to + 1e-10]) + 1, 
                     length(prob.vec))
    ...
    conf.int.title <- paste("\nPointwise Approximate ", 
                            conf.char," Confidence Intervals", sep = "")
    ...
    the.list <- get.parametric.quantiles(x, 
                                         prob.vec = prob.vec,
                                         conf.level = conf.level, 
                                         shape = shape)
    
    the.table <- cbind(p = the.list$prob.vec, 
                       Quanhat = the.list$tquanhat,
