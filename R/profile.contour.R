profile.contour <-
function (fitted, xlim = c(NA, NA), ylim = c(NA, NA),
    profile.title = paste(fitted$subtitle, "\n", lplot.type,
        "Likelihood for", variable.namex, "and", variable.namey,
        "\n", model.dist.str), variable.namex = fitted$xlab,
    variable.namey = fitted$ylab, transformationx = "linear",
    transformationy = "linear", original.par = F, levels = c(0.001,
        0.01, 0.2, 0.4, 0.6, 0.8, 0.9), pretty.x = NULL, pretty.y = NULL,
    add = F, lty = 1, col = 1, lwd = 1,...)
{
    ...
    if (!add) {
        xrna <- is.na(xlim)
        if (any(xrna))
            xlim[xrna] <- range(fitted$x)[xrna]
        yrna <- is.na(ylim)
        if (any(yrna))
            ylim[yrna] <- range(fitted$y)[yrna]
        #plot(xlim, ylim, type = "n", xaxt = "n", yaxt = "n",  xlab = "", ylab = "")
    }
    image2D(z = fitted$z, x = fitted$x, y = fitted$y, levels = levels, lty = lty,
        lwd = lwd, contour = TRUE, clab = c(expression(frac(bolditalic(L)[0],bolditalic(L)[MLE]))),
        xlab = parse(text = variable.namex), ylab = parse(text = variable.namey), 
        cex.lab = cex.lab)
    if (add)
        return(invisible())
    if (do.mixed.text) {
        mixed.mtext(side = 1, texts = variable.namex, line = 1.5,
            cex = 1.5 * cex.lab)
        mixed.mtext(side = 2, line = 2, texts = variable.namey,
            srt = 0, cex = 1.5 * cex.lab)
}   else {
