...
function (...)
{
    ...
    xdgets <- function(tvec, varzeta, sigma, distribution) {
        pgets(tvec, 0, sigma, varzeta, distribution)
    }
    lower <- matrix(c(-3, -3, -2, -2, -4, -3, -3, -3, -2),
                    byrow = T, 
                    ncol = 3)
    upper <- matrix(c(2, 3, 3, 3, 4, 2, 2, 3, 3),
                    byrow = T,
                    ncol = 3)
    for (j in 1:3) {
        for (i in 1:3) {
            ...
            matplot(tvec, 
                    outer(tvec, varzetavec, xdgets, 
                          sigma = sigmavec[i],distribution = distvec[j]), 
                    lty = 1:length(varzetavec),
