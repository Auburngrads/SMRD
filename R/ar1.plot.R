    ...
    ...) 
{
    ...
    plot(the.interarrival.times[-1], the.interarrival.times[-number.failures], 
        xlab = "", ylab = "", cex = 1.5, las = 1, ...)
    title(xlab = xlab, cex = 1.5)
    title(ylab = ylab, cex = 1.5, mgp = c(4, 1, 0))
    if (is.null(my.title)) 
        my.title <- get.data.title(data.rdu)
    title(my.title)
    acf.values <- acf(the.interarrival.times, plot = plot.acf)$acf[1:nacf]
    acf.tvalues <- acf.values * sqrt((length(the.interarrival.times) - 
        0:(nacf - 1)))
