...
function (multiple.mlest.out, prob.vec, number.times = 10, number.try = 15) 
{
    theta.hat <- unlist(lapply(multiple.mlest.out, function(sublist) {
        sublist[["theta.hat"]]
    }))
    log.of.data <- all(unlist(lapply(multiple.mlest.out, function(sublist) {
    ...
    for (i in 1:number.try) {
        ...
        else lower.quant.lower.try <- lower.quant.lower.approx - 
            0.25 * (upper.quant.lower.bound - lower.quant.lower.approx)
        time.vec <- seq(lower.quant.lower.try, upper.quant.lower.bound, 
            length = number.times)
        try.probs <- fx.mfmc.probs(theta.hat, multiple.mlest.out, 
            time.vec)
        if (F) {
            cat("quant approx=", format(lower.quant.lower.try), 
                format(lower.quant.lower.approx), format(upper.quant.lower.bound), 
                "\n")
        ...
        if (min(try.probs) < prob.range[1]) {
            lower.quant.lower.bound <- max(time.vec[try.probs < 
                prob.range[1]])
