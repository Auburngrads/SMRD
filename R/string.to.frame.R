...
function (the.string, col.names = NULL) 
{
    ...
    if (is.null(the.string) || nchar(the.string) <= 0 || the.string == 
        "None") 
        return(NULL)
    the.semi.vector <- ClistToVec(the.string, sep = ",")
    the.semi.vector <- the.semi.vector[nchar(the.semi.vector) > 
        0]
    the.number.of.columns <- length(ClistToVec(the.semi.vector[1], 
        ";"))
    the.char.mat <- matrix(NA, nrow = length(the.semi.vector), 
        ncol = the.number.of.columns)
    for (i in 1:length(the.semi.vector)) {
        the.row <- ClistToVec(the.semi.vector[i], ";")
        if (length(the.row) != the.number.of.columns) 
            stop(paste("String", i, the.semi.vector[i], "not consistent with string 1 having length", 
    ...
    for (j in 1:ncol(the.frame)) {
        ...
        else the.frame[[j]] <- factor(as.character(the.column))
    }
    if (!is.null(col.names)) 
        names(the.frame) <- col.names
    return(the.frame)
}
