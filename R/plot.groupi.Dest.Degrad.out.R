...
function (x, transformation.x, ylim = c(NA,
    ...
    cex = 1, xlab = NULL, ylab = NULL, lwd = 2, pch.vec = (1:(length(unique.stress.clist) +
        1))[-2], col.vec = (1:(length(unique.stress.clist) +
        1)), grids = F, power, focus.variable = 1, do.legend = "On plot",
    ...)
{
  CheckString <- function (pat, str) { return(regexpr(pat, str) > 0) }
    ...
    stress <- attr(x, "stress")[ok.values,
        , drop = F]
    ...
    model.string <- paste("Resp:", transformation.response, ",Time:",
        transformation.time, ",x:", paste(transformation.x, collapse = ","),
    ...
    if (is.null(xlab)) {
        if (generic.relationship.name(x.axis) == "Box-Cox") {
            ...
            x.axis.name <- paste(x.axis, "(", the.power, ")",
                sep = "")
}       else { x.axis.name <- transformation.x }
        xunits <- dimnames(xmat(data.ddd))[[2]]
        xlab <- paste(xunits[focus.variable], "on", fix.axis.name(x.axis.name),
            "Scale")
    }
    if (is.null(my.title))
    ...
    slope.lower <- sign.factor * attr(x,
        "slope.lower")[ok.values]
    slope.upper <- sign.factor * attr(x,
        "slope.upper")[ok.values]
    if (!all(ok.values))
    ...
    if (ncol(stress) > 1) {
        ...
        stress.clist <- apply(stress[, -focus.variable, drop = F],
            1, paste, sep = "", collapse = ";", the.names)
}   else {
    ...
    xrna <- is.na(xlim)
    if (any(xrna))
        xlim[xrna] <- range(stress)[xrna]
    expand.factor <- 1.000001
    xlim <- c(xlim[1]/expand.factor, xlim[2] * expand.factor)
    yrna <- is.na(ylim)
    if (any(yrna))
        ylim[yrna] <- range(slope, slope.lower, slope.upper)[yrna]
    if (is.null(xlab))
        xlab <- attr(data.ddd, "x.columns")
    plot.paper(x = xlim, y = ylim, x.axis = x.axis, y.axis = y.axis,
        xlab = xlab, ylab = "Slope", cex = cex, my.title = "",
        grids = grids, title.option = title.option, ...)
    delta.space <- (x.loc(1) - x.loc(0))/50
    for (i in 1:length(unique.stress.clist)) {
        the.ones <- stress.clist == unique.stress.clist[i]
        points.default(f.relationship(stress[the.ones, focus.variable],
            x.axis), f.relationship(slope[the.ones], y.axis),
        ...
        segments(f.relationship(stress[the.ones, focus.variable],
            x.axis) - delta.space, y.location.lower, f.relationship(stress[the.ones,
            focus.variable], x.axis) + delta.space, y.location.lower,
            pch = pch.vec[i], col = col.vec[i])
        y.location.upper <- f.relationship(slope.upper, y.axis)[the.ones]
        segments(f.relationship(stress[the.ones, focus.variable],
            x.axis) - delta.space, y.location.upper, f.relationship(stress[the.ones,
            focus.variable], x.axis) + delta.space, y.location.upper,
            pch = pch.vec[i], col = col.vec[i])
    }

        if (do.legend == "On plot" && length(unique.stress.clist) >
            1) {
            legend(x.loc(0.01), y.loc(0.98), unique.stress.clist,
                col = col.vec[1:length(unique.stress.clist)],
                pch = pch.vec[1:length(unique.stress.clist)], y.intersp = 0.675)
        }
