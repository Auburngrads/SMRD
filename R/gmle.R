gmle <-
function (data.ld, log.like, theta.start, model = NULL, f.origparam = function(thetatran,
    model) {
    theta <- thetatran
    return(theta)
    ...
    thetatran <- theta
    return(thetatran)
}, em.alg = NULL, special.stuff = NULL, func.call = match.call(),
    t.param.names = paste("Tparam", seq(1:length(theta.start)),
        sep = ""), orig.param.names = paste("Oparam", seq(1:length(theta.start)),
        sep = ""), max.fcal = 900, max.iter = 300, rfc.tol = 1e-15,
    xc.tol = 1e-13, digits = 5, fixed.param.list = NULL,debug1= 0,
    save.data.ld = T, ...)
{
    data.ld <- data.ld
    log.like <- log.like
    ...
    if (is.null(t.param.names))
        t.param.names <<- paste("thetatran", 1:number.parameters,
            sep = "")
    orig.param.names <<- model$orig.param.names
    if (is.null(orig.param.names))
        orig.param.names <<- paste("thetaorig", 1:number.parameters,
            sep = "")
    if (is.null(model$orig.param.names))
        model$orig.param.names <<- orig.param.names
    if (is.null(model$f.tranparam))
        model$f.tranparam <- f.tranparam
    ...
    if (map.SMRDDebugLevel() >= 4) {
       debug1<- get(envir = .frame0, "debug1")
        if (debug1< 3)
            assign(envir = .frame0,  inherits = TRUE,"debug1", 3)
    }
    ...
    if (!is.null(attr(data.ld, "sim.parameters")))
        theta.start <- attr(data.ld, "sim.parameters")
    if (is.null(em.alg)) {
        if (!is.null(fixed.param.list)) {
            ...
            profile.stable.parameters <- function(x.theta.hat,
                profile.on) {
            ...
            est.out <- wqm.nlmin(const.log.like, theta.opt, max.fcal = max.fcal,
                max.iter = max.iter, rfc.tol = rfc.tol, xc.tol = xc.tol)
            t.param <- est.out$x
            grad <- my.gradient(const.log.like, t.param)
            hessian <- my.hessian(const.log.like, t.param)
      } else {
            theta.start <- f.tranparam(theta.start, model)
            est.out <- wqm.nlmin(log.like, theta.start, max.fcal = max.fcal,
                max.iter = max.iter, rfc.tol = rfc.tol, xc.tol = xc.tol)
            t.param <- est.out$x
    ...
    gmle.out <- list(data.ld = data.ld, log.like = log.like,
        model = model, em.alg = em.alg, special.stuff = special.stuff,
        est.out = est.out, max.log.like = max.log.like, grad = grad,
        hessian = hessian, eigen.hessian = eigen.hessian, t.vcv = t.vcv,
        t.param = t.param, t.standard.error = t.standard.error,
        fixed.param.list = fixed.param.list, ierror = ierror)
    if (!is.null(f.origparam)) {
        genorigparm.out <- f.genorigparmvcv(t.vcv, t.param, f.origparam,
            model = model)
        origparam <- genorigparm.out$vec
        if (is.null(names(origparam)))
            names(origparam) <- model$orig.param.names
