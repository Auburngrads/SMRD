...
function (time.vec, distribution, FailLevel.tran, xuse.tran, 
    theta.hat) 
{
    ...
    if (theta.hat["beta1"] > 0) {
        answers <- 1 - wqmf.phibf((-time.vec - mu.quantile)/sigma.quantile, 
            distribution)
    }
    else {
        answers <- wqmf.phibf((time.vec - mu.quantile)/sigma.quantile, 
            distribution)
    }
    return(answers)
}
