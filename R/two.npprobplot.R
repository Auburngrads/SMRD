two.npprobplot <-
  function (data.ld, xlab = get.time.units(data.ld), xlim = c(NA,NA),
            ylim = c(NA, NA), time.vec = NULL, distribution.list = c('weibull', 'sev'),
            ...
            ...)
  {
    save.SMRD.options <- SMRDOptions(SMRD.DateOnPlot = F,
                                         SMRD.NameOnPlot = "")
    if (landscape) {
      old.par <- par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), err = -1)
    } else {
      old.par <- par(mfcol = c(1, 2), oma = c(0, 0, 0, 0), err = -1, mar = c(3.5, 3.5, 2.1, 0))
    }
    on.exit({
      par(mfrow = c(1,1))
      par(new = F)
