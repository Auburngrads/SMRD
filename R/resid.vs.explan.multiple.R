    ...
    x.to.plot = NULL, cex.points = 1, ...) 
{
    ...
    if (any(!on.list)) 
        stop(paste(x.to.plot[!on.list], "not in the x matrix"))
    for (i in 1:length(x.to.plot)) {
        resid.vs.explan(groupm.list = groupm.list, original.par = original.par, 
            cex = cex, my.title = my.title, grids = grids, xlim = xlim, 
            ylim = ylim, x.axis = x.axis, x.to.plot = x.to.plot[i], 
