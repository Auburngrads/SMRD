...
function (data.ddd, distribution, transformation.response = "log",
    transformation.time = "linear", ylim = c(NA, NA), xlim = c(NA,
        NA), my.title = NULL, ylab = NULL, xlab = NULL, cex = 0.5,
    cex.labs = 1, cex.points = 1, cex.tic.lab = 1, add = F, grids = F,
    title.option = NULL, pch.point = NULL, response.on.yaxis = T,
    group.var = 1:ncol(xmat(data.ddd)), subset = T, fail.level = NULL,
    plot.lines = T, do.legend = "On plot")
{
    ...
    model.string <- paste("Resp:", transformation.response, ",Time:",
        transformation.time, paste(", Dist:", distribution, sep = ""),
        sep = "")
    if (is.null(my.title))
        my.title <- paste(get.data.title(data.ddd), subset.name,
            "\n", model.string)
    if (length(group.var) == 1 && is.na(group.var)) {
        ...
        do.list(data.ddd) <- get.x.markers(data.ddd, group.var = group.var,
            long = T, include.complete = T)
        complete.list(data.ddd) <- complete.list(do.list(data.ddd))
    }
    hold.warn <- options(warn = 0)
    tmp.orig.param <- two.stage.dest.degrad(data.ddd, distribution = distribution,
    ...
    if (is.null(ylab))
        ylab <- get.response.units(data.ddd)
    if (is.null(xlab))
        xlab <- get.time.units(data.ddd)
    ...
    if (generic.relationship.name(transformation.response) ==
        "Arrhenius") {
    ...
    yrna <- is.na(ylim)
    if (any(yrna))
        ylim[yrna] <- range(Response(data.ddd))[yrna]
    xrna <- is.na(xlim)
    if (any(xrna))
        xlim[xrna] <- range(the.times)[xrna]
   Response(data.ddd) <- as.matrix(f.relationship(Response(data.ddd),
    ...
    two.stage.out <- two.stage.dest.degrad(data.ddd, distribution = distribution,
        double.count.zeros = F)
    ...
    for (i in 1:length(slope)) {
        if (map.SMRDDebugLevel() >= 4)
            cat(slope.computed.list[i], "Intercept=", intercept.trans[i],
                "slope=", slope.trans[i], "in groupi.Dest.Degrad.indivplots\n")
    }
    ...
    dummy.obs <- the.censor.codes == 0 | the.case.weights ==
        0
    if (is.null(pch.point))
        pch.point <- (1:(length(the.do.list) + 4))[-c(2, 6, 17,
            19)]
    if (length(pch.point) == 1)
        pch.point <- rep(pch.point, length(the.do.list))
    ...
    for (i in 1:length(the.do.list)) {
        ...
        if (!add)
            plot.paper(xlim = xlim, ylim = ylim,
                x.axis = transformation.time, y.axis = transformation.response,
                ylab = "", xlab = "", response.on.yaxis = response.on.yaxis,
                cex = cex, cex.labs = cex.labs, cex.tic.lab = cex.tic.lab,
                grids = grids)
        if (is.character(the.do.list[i]))
            mtext(text = the.do.list[i], side = 3, cex = 1, line = 1)
        ...
        if (response.on.yaxis) {
            if (any(ncensored))
                points.default(as.vector(the.times[ncensored]),
                  theResponse[ncensored, 1], pch = pch.point[i],
                  cex = (cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
            if (any(rcensored))
                points.default(as.vector(the.times[rcensored]),
                  theResponse[rcensored, 1], pch = 17, cex = (1.2 *
                    cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
            if (any(lcensored))
                points.default(as.vector(the.times[lcensored]),
                  theResponse[lcensored, 1], pch = 19, cex = (1.2 *
                    cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
            if (any(icensored))
                points.default(as.vector(the.times[icensored]),
                  (theResponse[icensored, 1] + theResponse[icensored,
                    2])/2, pch = 4, cex = (cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
}       else {
            if (any(ncensored))
                points.default(theResponse[ncensored, 1], as.vector(the.times[ncensored]),
                  pch = pch.point[i], cex = (cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
            if (any(rcensored)) {
                rh.eps <- 0.001
                rh.size <- 0.1
                arrows(theResponse[rcensored, 1] - rh.eps, as.vector(the.times[rcensored]),
                  theResponse[rcensored, 1] + rh.eps, as.vector(the.times[rcensored]),
                  size = (rh.size * cex * GetSMRDDefault("SMRD.point.size"))/100,
                  open = F, rel = F, col = the.color)
            }
            if (any(lcensored)) {
                rh.eps <- 0.001
                rh.size <- 0.1
                arrows(theResponse[lcensored, 1] + rh.eps, as.vector(the.times[lcensored]),
                  theResponse[lcensored, 1] - rh.eps, as.vector(the.times[lcensored]),
                  size = (rh.size * cex * GetSMRDDefault("SMRD.point.size"))/100,
                  open = F, rel = F, col = the.color)
            }
            if (any(icensored))
                points.default((theResponse[icensored, 1] +
                  theResponse[icensored, 2])/2, as.vector(the.times[icensored]),
                  pch = 4, cex = (cex.points * GetSMRDDefault("SMRD.point.size"))/100,
                  col = the.color)
        }
