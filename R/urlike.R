...
function (tvec, mut1, sigmat1, mut2, sigmat2, mur1, sigmar1, 
    mur2, sigmar2, rho) 
{
    zout <- .Fortran("surlike", tvec = as.double(tvec), nt = as.integer(length(tvec)), 
        mut1 = as.double(mut1), sigmat1 = as.double(sigmat1), 
        mut2 = as.double(mut2), sigmat2 = as.double(sigmat2), 
