...
function (mlest.out, prob.vec = as.numeric(c(".001", ".005",
    ".01", ".05", ".1", ".2", ".3", ".4", ".5", ".6", ".7", ".8",
    ".9", ".99")), conf.level, shape = NULL, do.ci = T)
{
    ...
    if (do.ci) {
        ...
        zmat <- cbind(1, zquan)
        vartquan <- diag(zmat %*% mlest.out$vcv %*% t(zmat))
        ...
        return(list(prob.vec = prob.vec, tquanhat = tquanhat,
            stderror = stderror, lower = lower, upper = upper))
