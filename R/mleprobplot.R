          ...
          param.loc = "bottomright",...)
{
    ...
    if (is.logdist(distribution)) {
        if (any(Response(data.ld) <= 0))
            stop("log-distribution specified but nonpositive response(s) in your life data object.")
    }
    mlest.out <- mlest(data.ld, distribution, theta.start = theta.start,
        parameter.fixed = parameter.fixed, gamthr = gamthr, ...)
    ...
    if (any(is.na(ybandrange)))
        ybandrange <- NULL
    xrna <- is.na(xlim)
    if (any(xrna))
        xlim[xrna] <- range(xbandrange, time.range, cdpoints.out$yplot)[xrna]
    yrna <- is.na(ylim)
    if (any(yrna))
        ylim[yrna] <- range(cdpoints.out$pplot, ybandrange)[yrna]
    if (band.type != "n") {
        conf.int.title <- paste("and Pointwise", paste(floor(conf.level *
            100 + 0.01), "%", sep = ""), "Confidence Intervals")
  } else {
        conf.int.title <- ""
    }
    if (is.null(my.title)) {
        my.title <- paste(get.data.title(data.ld), "\n", "with",
            distribution, "ML Estimate ", conf.int.title, trunc.correct.string)
    }
    if (!missing(gamthr) && is.null(sub.title)) {
        if (is.postsctiptok()) {
            sub.title <- paste("~f13~.g~f3~. = ", format(gamthr),
                "~", sep = "")
            cex <- 1.2
      } else {
    ...
    if (!add) {
        log.of.data <- probplot.setup(distribution, range(xlim),
            ylim, my.title = my.title, sub.title = sub.title,
            cex = cex, cex.labs = cex.labs, grids = grids, linear.axes = linear.axes,
            title.option = title.option, slope.axis = slope.axis,
            ylab = ylab, xlab = xlab, title.line.adj = title.line.adj,
            ...)
  } else {
        log.of.data <- get.prob.scales(distribution, shape = NULL,
            prob.range = ylim)$logger
    }
    plot.nonparametric.estimate(x = cdfest.out, cdpoints.out, distribution,
        log.of.data, how.show.fhat = "points", shape = NULL,
        point.cex = cex.points, pch = pch, point.pch = pch, xlim,
        cex.axis = cex.axis, cex.lab = cex.labs)
    bands.over <- bands$bands.over
    times <- bands$times
    fhat <- bands$fhat
    if (!is.null(fhat)) {
        lines(pp.data(times[bands.over], log.of.data), pp.quant(fhat,
            distribution)[bands.over], col = "black", lwd = lwd.fhat)
    }
    switch(band.method, zhat.cdf.method = , logit.cdf.method = {
      if (band.type != "n" && !is.null(ybandrange)) {
            plot.bands(bands, distribution = distribution, log.of.data = log.of.data,
                lwd.ci = lwd.ci, lwd.fhat = lwd.fhat)
        }
    }, log.quantile.method = {
        if (band.type != "n" && !is.null(xbandrange)) {
            plot.bands.quantile(bands, distribution = distribution,
                log.of.data = log.of.data, lwd.ci = lwd.ci, lwd.fhat = lwd.fhat,
                mono.tran = mono.tran)
        }
    }, {
        stop(paste(band.method, "not recognized"))
