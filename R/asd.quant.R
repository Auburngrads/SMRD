asd.quant <-
function (plan.values, n, censor.time, quantile.mark) 
{
    ...
    zout <- .Fortran("vavar", as.integer(numdist(distribution)), 
        as.integer(vlength), as.double(zc), as.double(ze), ans = double(vlength))
    return(sqrt(zout$ans/n) * plan.values$sigma)
}
