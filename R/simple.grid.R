          ...
          restrict.expand.range = F, ...)
{
    ...
    if (is.na(the.quantile)) {
        ...
        ci.quant <- c(theta.hat[1] - factor * sqrt(vcv.matrix[1,1]),
                      theta.hat[1] + factor * sqrt(vcv.matrix[1,1]))
        
      } else {
    
        xname <- paste("t[",
                       format(the.quantile, digits = 6),
                       "]","~","Quantile","~",
                       get.time.units(data.ld))
        
        zquant <- quant(the.quantile, distribution)
        quanthat <- theta.hat[1] + zquant * theta.hat[2]
        sequant <- sqrt(vcv.matrix[1, 1] + (zquant^2) * vcv.matrix[2,2] +
                                         2 * zquant * vcv.matrix[1,2])
        ci.quant <- c(quanthat - factor * sequant, 
                      quanthat + factor * sequant)
    ...
    ci.spread <- c(theta.hat[2] / exp((factor * sqrt(vcv.matrix[2,2])) / theta.hat[2]), 
                   theta.hat[2] * exp((factor * sqrt(vcv.matrix[2,2])) / theta.hat[2]))
    ...
    if (restrict.expand.range) {
        ...
        xrna <- is.na(xlim)
        if (any(xrna)) xlim[xrna] <- ci.quant[xrna]
        
        yrna <- is.na(ylim)
        if (any(yrna)) ylim[yrna] <- ci.spread[yrna]
        
  }
    
    likelihood.grid.out <- likelihood.grid(ls2.out, 
                                           the.quantile = the.quantile,
                                           range.list = list(xlim, ylim), 
                                           relative = T, 
                                           size = size,
                                           xname = xname, 
                                           yname = spread.name, 
