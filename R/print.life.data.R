print.life.data <-
function (x, includex = T, quote = F, prefix = "", digits = 4,...)
{
    ...
    if (is.null(the.case.weights) || all(the.case.weights ==
        1)) {
        name.case.weights <- NULL
        the.case.weights <- NULL
  } else name.case.weights <- "Case.weights"
    ...
    if (is.null(the.censor.codes) || all(the.censor.codes ==
        1)) {
    ...
    the.frame <- data.frame(cbind(Response(x), the.censor.codes,
        the.case.weights, the.failure.modes, trunc.status, truncation.response(x)))
    if (!is.null(xmat(x))) {
    ...
    names(the.frame) <- c(colnames(Response(x)), name.censor.code,
        name.case.weights, name.fail.modes, name.truncation.code,
        name.truncation.resp, dimnames(xmat(x))[[2]])
