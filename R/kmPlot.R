...
function (data.ld)
{
    ...
    segments(cdfest.out$p[over.interval], 
             1 - cdfest.out$prob[over.interval],
             cdfest.out$q[over.interval], 
             1 - cdfest.out$prob[over.interval])
    
    the.median <- approx(x = 1 - cdfest.out$prob[over.interval],
                         y = cdfest.out$p[over.interval], 
                         xout = 0.5)$y
    
    L90 <- approx(x = 1 - cdfest.out$prob[over.interval], 
                  y = cdfest.out$p[over.interval],
                  xout = 0.1)$y
    ...
    return(c(the.median = the.median, 
             L90 = L90, 
             number.cases = summary(data.ld)$number.cases))
