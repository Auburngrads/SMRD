...
function (x, the.quantile = NA, rel.or.conf = "Joint confidence region",
    ...
    show.mle = T, my.title = NULL, do.persp = F, direction = NULL,
    elevation = NULL, distance = NULL, original.par = T, add = add, lwd = 1,...)
{
    if (original.par) {
        old.par <- par()
        on.exit({
    ...
    distribution <- generic.distribution(attr(x,
        "distribution"))
    if ((distribution == "weibull" && x$xlab ==
        "mu") || x$log.quantile) {
        transformationx <- "log"
        if (distribution == "weibull" && x$xlab ==
            "mu") {
    ...
    if (do.persp) {
        if (is.null(my.title))
            my.title <- paste(data.title, "\n", dist.name, "Distribution Relative Likelihood")
        ...
        the.ranges <- sapply(x[c(1, 2, 3)],
            function(x) diff(range(x, na.rm = T)))
        if (map.SMRDDebugLevel() >= 4)
            cat("\nView=", paste(direction, elevation, distance,
                sep = ":"), "\n")
        likelihood.perspective(x, profile.title = "",
            transformationy = "linear", theta = theta, phi = phi,
            r = r)
  } else {
        if (rel.or.conf == "Joint confidence region") {
            ...
            xlim <- attr(x, "xlim")
            ylim <- attr(x, "ylim")
            conf.contour(x, xlim = xlim,
                ylim = ylim, profile.title = "", transformationx = transformationx,
                variable.namey = x$ylab, transformationy = "linear", lwd = lwd,
                levels = contour.indicators, add = add, original.par = F,...)
      } else {
            if (is.null(my.title))
                my.title <- paste(data.title, "\n", dist.name,
            ...
            profile.contour(fitted = x, profile.title = "",
                transformationx = transformationx, variable.namey = x$ylab,
                transformationy = "linear", levels = contour.indicators,
                add = add, original.par = F, lwd = lwd)
        }
        if (show.mle) {
