...
function (korder, nsamsize, timevec, mu, sigma, distribution) 
{
    zout <- .Fortran("postkp", as.integer(korder), as.integer(nsamsize), 
        as.double(timevec), as.integer(length(timevec)), as.double(mu), 
        as.double(sigma), as.integer(length(mu)), as.integer(numdist(distribution)), 
