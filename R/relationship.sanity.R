relationship.sanity <-
function (the.xmat, relationship, relat.string = "Relationship")
{
    problem <- F
    xnames <- dimnames(the.xmat)[[2]]
    if (length(relationship) != length(xnames))
        stop(paste("\n***The number of relationships", length(relationship),
            " is not equal to", length(xnames), "(the number of columns in the X matrix)\n",
