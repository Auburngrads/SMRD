          ...
          prefix = "",...)
{
    ...
    if (!is.null(the.analysis.type)) cat("\n", the.analysis.type, "\n", sep = "")
    
    response.units <- get.response.units(x$data.ld)
    if (response.units != "") cat(paste("Response units: ", response.units, "\n", sep = ""))
    
    time.units <- get.time.units(x$data.ld)
    if (time.units != "" && time.units != response.units) cat(paste("\nTime units: ", time.units, "\n", sep = ""))
    
    the.distribution <- x$model$distribution
    if (!is.null(the.distribution)) the.distribution <- distribution.name(x$model$distribution)
    if (!is.null(the.distribution)) cat("\n", the.distribution, " Distribution\n", sep = "")
    
    the.relationship <- x$relationship
    if (!is.null(the.relationship)) cat(paste("\nRelationship:", the.relationship, "\n"))
    ...
    if (print.vcv || is.null(x$origparam)) {
        ...
        if (!is.null(x$fixed.param.list$fixed.parameters)) {
            ...
            param.corr <- param.corr[-fixed.parameters, -fixed.parameters, drop = F]
