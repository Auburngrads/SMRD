          ...
          focus.variable = 1,...)
{
    ...
    plot.spec.title <- paste(my.title, 
                             model.string, 
                             "\nFailure time",
                             quantile.list[1], 
                             "quantile vs", 
                             xlab)
    sim.data.title <- paste("Simulated data from",
                            plan.string,
                            plan.values.string)
    ...
    model.string <- paste("x:", 
                          paste(ALT.plan.values$relationship, collapse = ","), 
                          ", Dist:", 
                          ALT.plan.values$distribution, sep = "")
    ...
    ALT.plot.time.v.x(ALTsim.full.out, 
                      quantile.list = quantile.list,
                      my.title = plot.spec.title, 
                      title.option = title.option,
                      grids = grids, 
                      numplotsim = numplotsim, 
                      nxpoints = nxpoints,
