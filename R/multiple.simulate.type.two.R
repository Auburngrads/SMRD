...
function (n, 
          ...
          number.sim = 1000)
{
    if (length(n) != length(r)) stop(paste("Length of n=", length(n), 
                                           "and r=", length(r), "need to be the same"))
    n.lt.2 <- n < 2
    if (any(n.lt.2)) stop(paste("Values of n must be greater than 1", n[n.lt.2]))
    r.lt.2 <- r < 2
    if (any(r.lt.2)) stop(paste("Values of r must be greater than 1", r[r.lt.2]))
    r.gt.n <- r > n
    if (any(r.gt.n)) stop(paste("Values of r", r[r.gt.n], "greater than n", n[r.gt.n]))
    results <- list()
