...
function (x, ylim = c(NA, NA), xlim = c(NA, NA),
    ...
    density.scale = NULL, density.at = "Automatic", add.density.at = NULL,
    quant.lines = c(0.1, 0.5, 0.9), cex = 1, nxpoints = 50, xlab = NULL,
    ylab = NULL, lwd = 2, grids = F, add = F, response.on.yaxis = T,
    include.data = T, ...)
{
    data.ld <- x$data.ld
    
    if(is.null(attr(data.ld,"xlabel"))) {
      xunits <- names(get.xlabel(data.ld))
    } else { 
    ...
    xrna <- is.na(xlim)
    if (any(xrna))
        xlim[xrna] <- range(xmat(data.ld)[, 1])[xrna]
    if (is.null(my.title))
    ...
    if (!add) {
        ...
        plot.paper(x = xlim, ylim, x.axis = x.axis, y.axis = y.axis,
    ...
    for (i in 1:length(quant.lines)) {
      quant.names[i] <- c(paste("t[",quant.lines[i],"]"))
        if (response.on.yaxis) {
            lines(f.relationship(rflm.model.lines.out$xvec[-nxpoints],
                x.axis), rflm.model.lines.out$yquant.mat[-nxpoints,
                i], lwd = lwd, col = i+1, lty = i)
#             text(f.relationship(rflm.model.lines.out$xvec[nxpoints],
#                 x.axis), rflm.model.lines.out$yquant.mat[nxpoints,
#                 i], percent.conf.level(quant.lines[i]), adj = -0.5)
      } else {
            lines(rflm.model.lines.out$yquant.mat[-nxpoints,
                i], f.relationship(rflm.model.lines.out$xvec[-nxpoints],
                x.axis), lwd = lwd, col = i+1, lty = i)
#             text(rflm.model.lines.out$yquant.mat[1, i], y.loc(0.05),
#                 percent.conf.level(quant.lines[i]), adj = 0.5)
        }
    }
    legend("topright", legend = parse(text = quant.names),lwd = lwd, 
           col = seq(2,length(quant.lines)+1),lty = seq(1,length(quant.lines)) , bty = "n")
    #title(main = my.title)
    if (include.data && !is.null(data.ld) &&is.onlist("life.data",
        oldClass(data.ld)))
        censored.data.plot(data.ld, x.axis = x.axis, y.axis = y.axis,
            add = T, title.option = title.option, my.title = my.title,
