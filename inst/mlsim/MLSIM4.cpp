...
//' 
//' kmeth approx method (1=poisson, 2=normal, 3=montecarlo
...
//' 
//' rhostr  rhohatstar (vector) computed from the simulation estimates
//' 
//' rhovec  rhohat (vector) computed from the data ml estimates
//' 
...
// Save prediction calibration stuff for the data sample
   prisk(kdist,theta.at(0),theta.at(1),nsamsz,centim,
         ngroup,prdelt,rhohat,krfail,nmrvec,crisk);
   ...
   for(int isim = 1; isim <= numsim; isim++){
       ...
       // Generate a staggered entry data set
          ...
          // Should give this a junk number below too, probably
             ...
             wqm_mlboth(x,y,cen,wt,nrow,nter,ny,nty,ty,
          ...
          // Save prediction calibration stuff for the boot sample
             prisk(kdist,thetah.at(0),thetah.at(1),nsamsz,centim,
                   ngroup,prdelt,rhostr,krfail,nmrvec,crisk);
