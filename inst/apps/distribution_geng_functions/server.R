...
C <- reactive({ SMRD::pgeng(t(), input$theta.geng, input$beta.geng, input$kappa.geng)})
P <- reactive({ SMRD::dgeng(t(), input$theta.geng, input$beta.geng, input$kappa.geng)})
R <- reactive({ 1-C()})
h <- reactive({ exp(log(P())-log(R()))})
H <- reactive({ -1*log(1-SMRD::pgeng(t(), input$theta.geng, input$beta.geng, input$kappa.geng))})
Q <- reactive({ SMRD::qgeng(p, input$theta.geng, input$beta.geng, input$kappa.geng)})
