...
egeng.C <- reactive({ SMRD:::pegengl(egeng.t(), input$mu.egeng, input$sig.egeng, input$lam.egeng)})
egeng.P <- reactive({ SMRD:::degengl(egeng.t(), input$mu.egeng, input$sig.egeng, input$lam.egeng)})
egeng.R <- reactive({ 1-egeng.C()})
egeng.h <- reactive({ exp(log(egeng.P())-log(egeng.R()))})
egeng.H <- reactive({ -1*log(1-SMRD:::pegengl(egeng.t(), input$mu.egeng, input$sig.egeng, input$lam.egeng))})
egeng.Q <- reactive({ SMRD:::qegengl(egeng.p, input$mu.egeng, input$sig.egeng, input$lam.egeng)})
